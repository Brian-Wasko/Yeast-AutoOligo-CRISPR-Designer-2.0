...
const CopyToClipboardButton: React.FC<{ textToCopy: string }> = ({ textToCopy }) => {
    ...
    return (
        <button
            onClick={handleCopy}
            className={`absolute top-2 right-2 flex items-center gap-1.5 px-2 py-1 text-xs rounded-md transition-all ${
                copied 
        ...
        >
            ...
            <span className="font-sans font-medium">{copied ? 'Copied!' : 'Copy'}</span>
        </button>
    );
};
...
export const ResultCard: React.FC<ResultCardProps> = ({ result, index }) => {
  ...
  const renderScoreBadge = () => {
    ...
    return (
        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold border ml-2 ${colorClass}`}>
            Score: {score} ({label})
        </span>
    );
  };
        ...
        <div className="space-y-3">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <div className="bg-slate-800 rounded-lg p-3 group relative">
              <CopyToClipboardButton textToCopy={result.cloningOligoA} />
              <span className="text-xs text-slate-400 block mb-1">Oligo A</span>
              <code className="text-indigo-300 text-sm font-mono break-all">{result.cloningOligoA}</code>
            </div>
            <div className="bg-slate-800 rounded-lg p-3 relative">
               <CopyToClipboardButton textToCopy={result.cloningOligoB} />
               <span className="text-xs text-slate-400 block mb-1">Oligo B</span>
              <code className="text-indigo-300 text-sm font-mono break-all">{result.cloningOligoB}</code>
            </div>
          </div>
        </div>

        <div className="space-y-3">
          <h4 className="text-sm font-semibold text-slate-900 uppercase tracking-wide">Genomic Repair Template (100bp)</h4>
          <div className="space-y-2">
            <div className="bg-slate-100 rounded-lg p-3 border border-slate-200 relative">
                <CopyToClipboardButton textToCopy={result.repairTemplate.toUpperCase()} />
                <span className="text-xs text-slate-500 block mb-1">Forward (Oligo A)</span>
                <code className="text-slate-700 text-xs font-mono break-all leading-relaxed">
                    {renderSeqWithHighlights(result.repairTemplate, false)}
                </code>
            </div>
             <div className="bg-slate-100 rounded-lg p-3 border border-slate-200 relative">
                <CopyToClipboardButton textToCopy={result.repairTemplateRevComp.toUpperCase()} />
                <span className="text-xs text-slate-500 block mb-1">Reverse Complement (Oligo B)</span>
                <code className="text-slate-700 text-xs font-mono break-all leading-relaxed">
                    {renderSeqWithHighlights(result.repairTemplateRevComp, true)}
                </code>
            </div>
          </div>
        </div>
